...
func ProjectWizardAskForExistingPersistentVolumeClaim() (namespace string, err error) {
	...
	if err != nil {
		logoutput.AddErrorEntryAndDetails("  -> Unable to get persistent volume claim.", err.Error())
...
func ProcessTemplatePvcExistingClaim(projectDirectory string, pvcName string) (success bool, err error) {
	...
	if files.FileOrDirectoryExists(jenkinsHelmValuesFile) {
		...
		if !successful || err != nil {
			logoutput.AddErrorEntryAndDetails("  -> Unable to replace PVC name in file ["+jenkinsHelmValuesFile+"]", err.Error())
			log.Error("[ProcessTemplatePvcExistingClaim] Unable to replace PVC name in file [%v], \n%v", jenkinsHelmValuesFile, err)
			return false, err
		}
	}
	...
	if files.FileOrDirectoryExists(pvcValuesFile) {
		...
		if !successful || err != nil {
			logoutput.AddErrorEntryAndDetails("  -> Unable to replace PVC name in file ["+pvcValuesFile+"]", err.Error())
			log.Error("[ProcessTemplatePvcExistingClaim] Unable to replace PVC name in file [%v], \n%v", pvcValuesFile, err)
			return false, err
		}
	}
