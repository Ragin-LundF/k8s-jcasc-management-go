...
func PersistenceVolumeClaimInstall(namespace string) (err error) {
	...
	log.Info("[PVC Install] Check if PVC should be installed on namespace [%v]", namespace)
	...
	if files.FileOrDirectoryExists(pvcClaimValuesFilePath) {
		...
		log.Info("[PVC Install] Kubernetes PVC specification found for namespace [%v]...", namespace)
		...
		log.Info("[PVC Install] Checking if PVC [%v] is already available for namespace [%v].", pvcName, namespace)
		...
		if !pvcExists {
			...
			log.Info("[PVC Install] PVC [%v] does not exist in namespace [%v]. Trying to install it...", pvcName, namespace)
			...
			if _, err := kubectl.ExecutorKubectl("apply", kubectlCmdArgs); err != nil {
				...
				log.Error("[PVC Install] Cannot create PVC [%v] for namespace [%v]", pvcName, namespace)
				return err
			}

			loggingstate.AddInfoEntry("  -> PVC [" + pvcName + "] does not exist in namespace [" + namespace + "]. Trying to install it...done")
			log.Info("[PVC Install] PVC [%v] does not exist in namespace [%v]. Trying to install it...done", pvcName, namespace)
		} else {
			loggingstate.AddInfoEntry("  -> PVC [" + pvcName + "] in namespace [" + namespace + "] found. No need to install it...")
			log.Info("[PVC Install] PVC [%v] in namespace [%v] found. No need to install it...", pvcName, namespace)
		}
	}
...
func readPvcNameFromFile(pvcClaimValuesFilePath string) (pvcName string, err error) {
	...
	if err != nil {
		...
		log.Error("Unable to read pvc file [%v]...\n%v", pvcClaimValuesFilePath, err)
		return pvcName, err
	}
	...
	if err != nil {
		...
		log.Error("Unable to unmarshal pvc file [%v]...\n%v", pvcClaimValuesFilePath, err)
		return pvcName, err
	}
